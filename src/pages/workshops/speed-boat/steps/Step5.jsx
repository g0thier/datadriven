/**
 * @module workshops/speed-boat/steps/Step5
 * @description Speed Boat step 5 screen for displaying challenge and objective context.
...
import speedBoatSkyImg from "../../../../assets/workshops/speed-boat/speed-boat-sky.png";
import WorkshopStepLayout from "../../WorkshopStepLayout.jsx";

const initialLeverCreationByParticipant = new Set();

/**
 * Renders Speed Boat step 5 (lever identification context).
 *
...
export default function Step5({ step, sessionTitle, collaboration, session }) {
  const myLeverNotes = collaboration?.myLeverNotes || [];
  ...
  useEffect(() => {
    if (isLoading) return;
    if (myLeverNotes.length > 0) return;
    if (typeof addLeverNote !== "function") return;
    if (!sessionId || !participantId) return;
    if (initialLeverCreationByParticipant.has(initialNoteKey)) return;

    initialLeverCreationByParticipant.add(initialNoteKey);

    let isCancelled = false;

    const createInitialNote = async () => {
      const createdNoteId = await addLeverNote({ text: "" });
      if (!createdNoteId && !isCancelled) {
        initialLeverCreationByParticipant.delete(initialNoteKey);
      }
    };
  ...
  }, [addLeverNote, initialNoteKey, isLoading, myLeverNotes.length, participantId, sessionId]);

  const handleChange = (noteId, value) => {
    if (isLoading) return;

    const currentValue = String(myLeverNotes.find((note) => note.id === noteId)?.text || "");
    if (currentValue === value) return;

    updateLeverNoteText?.(noteId, value);
  };
  ...
  const removeNote = (noteId) => {
    if (isLoading) return;
    if (myLeverNotes.length <= 1) return;
    removeLeverNoteAction?.(noteId);
  };
  ...
  return (
    ...
    >
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {myLeverNotes.map((note, index) => {
          const isLast = index === myLeverNotes.length - 1;

          return (
            <div
              key={note.id}
              className="relative bg-blue-100 rounded-lg shadow-md p-4 min-h-37.5 flex flex-col"
            >
              <textarea
                className="flex-1 bg-transparent resize-none focus:outline-none text-gray-800"
                placeholder="Écrivez un levier..."
                value={note.text || ""}
                onChange={(event) => handleChange(note.id, event.target.value)}
              />

              {myLeverNotes.length > 1 && (
                <button
                  type="button"
