...
export default function Step5({ step, sessionTitle, collaboration }) {
  ...
  const solutions = Array.isArray(collaboration?.activeSolutions) ? collaboration.activeSolutions : [];
  const selectedDefect = collaboration?.selectedDefect || null;
  const votesByItem = collaboration?.solutionVotesByItem || {};
  const currentParticipantId = collaboration?.participant?.id || "";
  const remainingVotes = Number.isFinite(collaboration?.remainingSolutionVotes)
    ? collaboration.remainingSolutionVotes
    : 0;
  ...
  const toggleVote = (solutionId, hasMine) => {
    ...
    collaboration?.actions?.toggleSolutionVote?.(solutionId);
  };
        ...
        <>
          ...
          <div className="bg-white rounded-2xl shadow-md p-6">
            <div className="flex items-center justify-between gap-3 mb-4">
              <div>
                <h3 className="text-base font-semibold text-gray-800">Vote sur les solutions</h3>
            ...
            {solutions.length === 0 ? (
              <p className="text-sm text-gray-500">Aucune solution disponible pour voter.</p>
            ) : (
              <ul className="list-disc pl-5 space-y-3 marker:text-gray-400">
                {solutions.map((solution) => {
                  const stickerSet = votesByItem[solution.id] || new Set();
