...
function insertModulePath(tree, item, linktoFn) {
    const candidates = [item.longname, item.name].filter(Boolean);
    let parts = [];

    candidates.some((value) => {
        let normalized = String(value).replace(/^module:/, '');

        // Handles values such as "utils/management/permissions.module:defaults".
        normalized = normalized.replace(/\.module:/g, '/');
        // Treat "." as module path delimiter to support tags like "permissions.defaults".
        normalized = normalized.replace(/\./g, '/');
        normalized = normalized.replace(/[#~].*$/, '');

        const candidateParts = normalized.split('/').filter(Boolean);
    ...
    parts.forEach((part, idx) => {
        ...
        if (isLeaf) {
            node.anchor = linktoFn(item.longname, part);
            return;
        }
...
function buildSidebarMembers({
    ...
    if (items.length) {
        if (sectionName === SECTION_TYPE.Modules) {
            items.forEach(function (item) {
                if (!hasOwnProp.call(itemsSeen, item.longname)) {
                    insertModulePath(navProps.items, item, linktoFn);
